...
def extract_pattern_from_instructions(instructions: list[str]) -> str:
    ...
    This is a simple heuristic pattern extraction.
    ...
    if len(set(instructions)) == 1:
        # All same, just return the escaped string
        return re.escape(instructions[0])
    ...
    # Build pattern (avoid overlap between prefix and suffix)
    if prefix and suffix:
        # Check if prefix and suffix overlap or are the same
        if prefix == suffix:
            # Same string, just use one
            return re.escape(prefix)
        elif len(prefix) + len(suffix) > len(instructions[0]):
            # Overlap, just use prefix
